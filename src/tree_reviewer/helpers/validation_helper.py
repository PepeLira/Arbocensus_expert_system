...
class ValidationHelper:
    ...
    def validate_tree_image(tree_image):
        error_tags = []
        if tree_image.hypothesis_score < get_env("HYPOTHESIS_THRESHOLD"):
            error_tags.append("Low Hypothesis score")
        if tree_image.classification_score < get_env("TREE_VALIDATION_SCORE"):
    ...
    def validate_metrics(tree_metrics):
        error_tags = []
        if tree_metrics.dap < get_env("LOW_DAP"):
            error_tags.append("Bad tree DAP")
        if tree_metrics.dap > get_env("HIGH_DAP"):
            error_tags.append("Bad tree DAP")
        if tree_metrics.tree_height < get_env("LOW_HEIGHT"):
            error_tags.append("Bad tree height")
        if tree_metrics.tree_height > get_env("MAX_HEIGHT"):
            error_tags.append("Bad tree height")
        if tree_metrics.principal_branches_h < get_env("LOW_BRANCHES"):
            error_tags.append("Bad branches height")
        if tree_metrics.principal_branches_h > get_env("MAX_BRANCHES"):
            error_tags.append("Bad branches height")
        return error_tags